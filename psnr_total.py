...
if os.path.exists('main_config.ini'):
    ...
    NUM_SAMPLES = main_config.getint('SSIM', 'num_samples', fallback=3)
else:
...
def arcs_detection(image, theta_e, delta_pix, umbral=0.05):
...
def visualizar_ejemplo_ssim(ds_orig, ds_pred, delta_pix, num_samples, path_saved):
    ...
    for ((img_o, te_o), (img_p, te_p)) in zip(ds_orig, ds_pred):
        ...
        im_real = img_o.numpy()[0].squeeze()
        im_pred = img_p.numpy()[0].squeeze()
        ...
        val_te_real = float(te_o.numpy()[0])
        val_te_pred = float(te_p.numpy()[0])
        
        im_real_norm = normalize_minmax(im_real)
        im_pred_norm = normalize_minmax(im_pred)
        
        if ssim_sk is not None:
            score, ssim_map = ssim_sk(im_real_norm, im_pred_norm, full=True, data_range=1.0, win_size=7)
        else:
            score = float(calculate_ssim(im_real_norm, im_pred_norm))
            ssim_map = np.abs(im_real_norm - im_pred_norm)
    ...
    print(f"{GREEN}Matriz de arcos y mapas SSIM guardados en: {path_saved}{ENDC}")
...
def main():
    ...
    ds_orig = load_tfrecord_dataset(orig_path)
    ds_pred = load_tfrecord_dataset(pred_path)

    if ds_orig is None or ds_pred is None:
        print(f"{RED}Error loading TFRecords for SSIM analysis from {PATH}{ENDC}")
        return

    print(f"{YELLOW}Processing model {KEY} for SSIM and PSNR evaluation...{ENDC}")
    
    for (img_o, tE_o), (img_p, tE_p) in zip(ds_orig, ds_pred):
        im1 = img_o.numpy()[0].squeeze()
        im2 = img_p.numpy()[0].squeeze()
        tE = float(tE_o.numpy()[0])

        n1 = normalize_minmax(im1)
        n2 = normalize_minmax(im2)
        
        # 1. SSIM Global
        s_global = float(calculate_ssim(n1, n2))
        
        # 2. SSIM Anillo de Einstein
        mask_ring = get_einstein_ring_mask(n1.shape, tE, DELTA_PIX)
        s_ring = float(calculate_ssim(n1 * mask_ring, n2 * mask_ring))

        metrics[KEY]['ssim_global'].append(s_global)
        metrics[KEY]['ssim_ring'].append(s_ring)
        
        m_val = mse_masked(n1, n2)
        metrics[KEY]['mse'].append(m_val)
        if m_val > 0:
            metrics[KEY]['psnr'].append(10 * np.log10(1.0 / m_val))
        else:
            metrics[KEY]['psnr'].append(100.0)

    print(f"{GREEN}Resultados Modelo {KEY}:{ENDC}")
    if metrics[KEY]['ssim_global']:
        ...
        print(f"  > Median PSNR: {np.median(metrics[KEY]['psnr']):.2f} dB")
